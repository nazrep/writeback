export default function Content() {
  return (
    <>
      <p>
        RTV Euro AGD to jeden z największych sieci sprzedaży elektroniki i AGD w Polsce. Niezależnie od tego, czy kupiłeś lodówkę, pralkę, telewizor czy smartfon — jako konsument masz <strong>2 lata ustawowej ochrony</strong> na podstawie ustawy o prawach konsumenta. Gwarancja producenta i wewnętrzne procedury RTV Euro AGD nie zastępują Twoich ustawowych praw.
      </p>

      <h2>Podstawa prawna reklamacji w RTV Euro AGD</h2>
      <p>
        Art. 43b ustawy o prawach konsumenta (Dz.U. 2014 poz. 827 z późn. zm.) zobowiązuje sprzedawcę do dostarczenia towaru zgodnego z umową. Jeśli w ciągu 2 lat od zakupu wystąpi wada — mechaniczna, elektryczna, fabryczna — możesz złożyć reklamację bezpośrednio w RTV Euro AGD jako sprzedawcy, bez potrzeby kontaktowania się z producentem.
      </p>
      <p>
        Przez cały 2-letni okres obowiązuje <strong>domniemanie wady przy zakupie</strong> (art. 43c ust. 1 UPK). To oznacza, że to sklep musi udowodnić, że wada powstała z Twojej winy — nie Ty musisz udowadniać, że produkt był wadliwy od początku.
      </p>

      <h2>Czego możesz żądać</h2>
      <p>Na podstawie art. 43c–43d ustawy o prawach konsumenta możesz wybrać między:</p>
      <ul>
        <li><strong>Naprawą</strong> — bezpłatna, w rozsądnym czasie, bez nadmiernych niedogodności dla Ciebie</li>
        <li><strong>Wymianą</strong> na identyczny lub równorzędny produkt wolny od wad</li>
        <li><strong>Obniżeniem ceny</strong> — gdy naprawa lub wymiana jest niemożliwa lub sklep odmawiał</li>
        <li><strong>Zwrotem pieniędzy</strong> — gdy wada jest istotna albo sklep już naprawiał i wada wróciła</li>
      </ul>
      <p>
        Sklep sam decyduje o kolejności (naprawa lub wymiana), ale musi być to wykonane bezpłatnie i bez zbędnej zwłoki. Jeśli po naprawie wada wróci — możesz od razu żądać wymiany lub zwrotu pieniędzy.
      </p>

      <h2>14-dniowy termin na odpowiedź</h2>
      <p>
        RTV Euro AGD ma <strong>14 dni kalendarzowych</strong> na ustosunkowanie się do reklamacji (art. 7a UPK). Liczenie zaczyna się od dnia dostarczenia pisma do sklepu. Jeśli nie odpowie — reklamacja jest uznana za zasadną z mocy prawa. To jeden z najważniejszych przepisów — korzystaj z niego.
      </p>

      <h2>Jak złożyć reklamację w RTV Euro AGD</h2>
      <ul>
        <li>Przygotuj dowód zakupu: paragon, fakturę, potwierdzenie zamówienia z konta rtveuроagd.pl</li>
        <li>Opisz wadę: datę wystąpienia, objawy, czy sprzęt był użytkowany zgodnie z instrukcją</li>
        <li>Jasno określ żądanie: naprawa, wymiana lub zwrot</li>
        <li>Powołaj się na art. 43b i 43c ustawy o prawach konsumenta</li>
        <li>Wyślij e-mailem lub złóż osobiście w sklepie, zachowując kopię i potwierdzenie odbioru</li>
      </ul>
      <p>
        Dla sprzętu AGD (pralka, lodówka, zmywarka) pamiętaj, że sklep lub serwis musi zorganizować odbiór wadliwego sprzętu na własny koszt — nie musisz go dowozić ani płacić za transport.
      </p>

      <h2>Typowe odpowiedzi RTV Euro AGD i jak na nie reagować</h2>
      <p>
        <strong>"Proszę zgłosić się do autoryzowanego serwisu producenta."</strong><br/>
        To błędna praktyka. Masz prawo złożyć reklamację do sklepu. Możesz skorzystać z serwisu producenta w ramach gwarancji, ale nie musisz — to Twój wybór.
      </p>
      <p>
        <strong>"Naprawa jest poza gwarancją."</strong><br/>
        Gwarancja producenta to co innego niż Twoje prawa ustawowe. Reklamacja z tytułu niezgodności z umową trwa 2 lata niezależnie od gwarancji producenta i jej warunków.
      </p>
      <p>
        <strong>"Wada powstała na skutek użytkowania."</strong><br/>
        Sklep musi to udowodnić ekspertyzą techniczną. Samo twierdzenie słowne lub e-mailowe to za mało. Do momentu przedstawienia ekspertyzy działa domniemanie wady z art. 43c ust. 1 UPK.
      </p>

      <h2>Gdy sklep nie reaguje</h2>
      <p>
        Brak odpowiedzi po 14 dniach to uznanie reklamacji. Jeśli mimo to sklep nie realizuje Twojego żądania, masz prawo:
      </p>
      <ul>
        <li>Złożyć skargę do <strong>Inspekcji Handlowej</strong> właściwej dla siedziby sklepu</li>
        <li>Zgłosić się do <strong>Rzecznika Praw Konsumentów</strong> w swoim powiecie</li>
        <li>Wejść na drogę sądową — roszczenia do 20 000 zł rozpatruje sąd w trybie uproszczonym</li>
      </ul>

      <h2>Ubezpieczenie i gwarancja rozszerzona od RTV Euro AGD</h2>
      <p>
        Wiele osób kupuje w RTV Euro AGD rozszerzoną gwarancję lub ubezpieczenie sprzętu. <strong>Nie rezygnuj z ustawowych praw na rzecz gwarancji rozszerzonej.</strong> Traktuj gwarancję rozszerzoną jako uzupełnienie, nie zamiennik. Jeśli wada mieści się w 2-letnim terminie ustawowym — korzystaj z reklamacji ustawowej.
      </p>
    </>
  );
}
